...
export default function TreeRenderer({
  ...
  const layout = useMemo(() => {
    ...
    if (!rootData) {
      if (import.meta.env?.DEV && treeData.nodes?.length) {
        // Surface the silent-blank instead of hiding it: a set_tree payload had nodes
        // but produced no rooted hierarchy (unresolved root or unlinked edges).
        console.warn('[TreeRenderer] set_tree produced no renderable tree — check root/edges:', treeData);
      }
      return null;
    }
